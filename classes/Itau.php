...
{
	// URL's
	public $shopline_url = 'https://shopline.itau.com.br/shopline/shopline.aspx';
	public $ticket_url = 'https://shopline.itau.com.br/shopline/Itaubloqueto.aspx';
	public $request_url = 'https://shopline.itau.com.br/shopline/consulta.aspx';
	
	// Credenciais
	public $codigoEmpresa;
	public $chave = "SEGUNDA12345ITAU";
	...
	function __construct($codigoEmpresa, $chave)
	{
		$this->sbox = array();
		$this->key = array();
		...
		if (strlen($codigoEmpresa) != $this->TAM_COD_EMP) {
			throw new ItauException('Código da empresa não está corretamente configurado.', 300);
		}
		
		if (strlen($chave) != $this->TAM_CHAVE) {
			throw new ItauException('tamanho da chave da chave diferente de 16 posições.');
		}
	...
	//$pedido,$valor,$observacao,$nomeSacado,$codigoInscricao,$numeroInscricao,$enderecoSacado,$bairroSacado,$cepSacado,$cidadeSacado,$estadoSacado,$dataVencimento,$urlRetorna,$obsAd1,$obsAd2,$obsAd3
	public function geraDados($paramString2, $paramString3, $paramString4, $paramString6, $paramString7, $paramString8, $paramString9, $paramString10, $paramString11, $paramString12, $paramString13, $paramString14, $paramString15, $paramString16, $paramString17, $paramString18)
	{
		$paramString1 = $this->codigoEmpresa;
		$paramString5 = $this->chave;

		if ((strlen($paramString2) < 1) || (strlen($paramString2) > 8)) {
			throw new ItauException('Número do pedido inválido.');
		}

		if (is_numeric($paramString2)) {
			$paramString2 = $this->PreencheZero($paramString2, 8);
		} else {
			throw new ItauException('Número do pedido não é numérico.');
		}

		if ((strlen($paramString3) < 1) || (strlen($paramString3) > 11)) {
			throw new ItauException('Valor da compra inválido.');
		}
		...
		if ($i !== FALSE) {
			...
			if (!is_numeric($str3)) {
				throw new ItauException('valor decimal não é numérico.');
			}

			if (strlen($str3) != 2) {
				throw new ItauException('valor decimal da compra deve possuir 2 posições após a virgula.');
			}

			$paramString3 = substr($paramString3, 0, strlen($paramString3) - 3) . $str3;
		} else {
			if (!is_numeric($paramString3)) {
				throw new ItauException('valor da compra não é numérico.');
			}

			if (strlen($paramString3) > 8) {
				throw new ItauException('valor da compra deve possuir no máximo 8 posições antes da virgula.');
			}
		...
		if (($paramString7 != "02") && ($paramString7 != "01") && ($paramString7 != "")) {
			throw new ItauException('código de inscrição inválido.');
		}

		if (($paramString8 != "") && (!is_numeric($paramString8)) && (strlen($paramString8) > 14)) {
			throw new ItauException('número de inscrição inválido.');
		}

		if (($paramString11 != "") && ((!is_numeric($paramString11)) || (strlen($paramString11) != 8))) {
			throw new ItauException('cep inválido.');
		}

		if (($paramString14 != "") && ((!is_numeric($paramString14)) || (strlen($paramString14) != 8))) {
			throw new ItauException('data de vencimento inválida.');
		}

		if (strlen($paramString16) > 60) {
			throw new ItauException('observação adicional 1 inválida.');
		}

		if (strlen($paramString17) > 60) {
			throw new ItauException('observação adicional 2 inválida.');
		}

		if (strlen($paramString18) > 60) {
			throw new ItauException('observação adicional 3 inválida.');
		}
		...
		$str2 = $this->Algoritmo($paramString1 . $str1, $this->chave);
	...
	public function geraCripto($paramString1, $paramString2, $paramString3)
	{
		if (strlen($paramString1) != $this->TAM_COD_EMP) {
			throw new ItauException('tamanho do codigo da empresa diferente de 26 posições.');
		}
		...
		if ($paramString2 == "") {
			throw new ItauException('código do sacado inválido.');
		}
	...
	public function geraConsulta($paramString1, $paramString2, $paramString3, $paramString4)
	{
		...
		if (is_numeric($paramString2)) {
			...
			throw new ItauException('numero do pedido não é numérico.');
		}

		if (($paramString3 != "0") && ($paramString3 != "1")) {
			throw new ItauException('formato inválido.');
		}
	...
	public function decripto($paramString1, $paramString2)
	{
		...
		$this->codigoEmpresa = substr($str, 0, 26);
	...
	public function geraDadosGenerico($paramString1, $paramString2, $paramString3)
	{
		...
		if (strlen($paramString1) != $this->TAM_COD_EMP)
		{
			throw new ItauException('tamanho do código da empresa diferente de 26 posições.');
		}
		...
		{
			throw new ItauException('sem dados.');
		}
