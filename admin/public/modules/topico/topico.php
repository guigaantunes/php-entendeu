<?php
    ...
    $materia = $classmateria->getById($idMateria);
?>

<div class="page-usuarios page">
    <div class="header-table">
		<h1><?=$nomeModulo?> (Matéria: <?=$materia['titulo']?>)</h1>
		<div class="acoes">
			<a href="<?=$pathModulo?>form.php?idMateria=<?=$idMateria?>" class="ajax-popup-link mfp-large btn-flat waves-effect waves-darken add cor-admin-text text-cor-detalhe"><i class="material-icons left">&#xE146;</i>Novo</a>
		</div>
	</div>
...
<script>
	$(function() {
	    ...
	    var table = $('.data-table').DataTable({
	        ...
	        colReorder: true,
	        ajax: "<?=$pathModulo?>ajax.php?idMateria=<?=$idMateria?>",
	        dom: 'Bfrtip',
	        stateSave: true,
	        buttons: [
		        {
