    ...
    $('#pagar').attr('disabled', 'disabled');
      ...
      var tokenResponseHandler = function(data) {
          ...
          if (data.errors) {
            ...
            showToast("Efetuando cobrança... " + JSON.stringify(data.errors));
              $("#token").val( data.id );
              $.ajax({
                  type: 'POST',
                  ...
                  success: function(data) {
                    ...
                    console.log(data);
                    if(obj.mensagens[0]==false){
                      showToast("Erro na cobrança, verifique seu cartão");
                      $('#pagar').removeAttr('disabled');
                    }
                    else{
                      showToast("Cobrança efetuada, redirecionando ...");
